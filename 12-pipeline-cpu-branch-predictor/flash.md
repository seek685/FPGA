...
优化:
    ...
    改变：把jal单独提前 此前的next_pc只是放在组合逻辑里的case一起判断 现在拆出来分支以及jalr和jal的选择信号 然后判断这些选择信号之间的联系
    5，增加分支预测器 4096的PHT深度还有12位宽度索引4096，GHR也是12位 BTB是512的表项 9位宽作为索引 然后剩下的21位高位作为匹配检测该指令是否有被训练记录下来 是否匹配  BTB用3个不同位宽的数组去组成这个表项 代表高位匹配 valid与否 也就是是否有记录 还有跳转地址 当BTB命中之后 才会取PHT和改写GHR 至于怎么在IF阶段得到指令是否是分支指令 只需要在顶层提前译码发给预测器即可 
    在一开始表项全是空的 在没有分支之前 CPU照常运行 当面对第一个分支指令的时候 由于没有训练 那么这时候一定会跳过 然后经历ex的冲刷的同时 更新GHR以及BTB PHT 当后续训练完成后 如果低位PC命中BTB 然后高位也命中说明该指令有被记录在案 那么后续直接取出BTB的跳转地址并且在下一个周期更新为next_pc 直接预测 当表项训练充分的话 预测率很高 直接舍去了高频的冲刷乘法 提高性能 
    对于PHT和BTB PHT远多于BTB因为一条指令可能同时有多种跳转情况 而但是跳转的地址是固定的 只是在不同的情况下可能跳 可能不跳PC都是一样的GHR不一样 得到的PHT结果自然不一样 而且更多 
    在没有冲刷 而且是分支指令的时候 预测器正常工作
    当分支预测错误的时候或者还有一种情况就是jalr指令下一条是b型指令 这时候肯定会污染GHR 因为jalr到ex之后直接跳转 但是在此之前b指令已经更改了GHR 这时候恢复的方法就不一样了 应该是原先的GHR直接覆盖 而不是正常的b型训练时的覆盖并且修正 也就是左移
    这里为什么没有包括jal呢 因为jal指令我提前到了ID阶段执行 并且得等到前一条指令译码完是不是id_jal 才会决定是不是要更新下一条的组合逻辑改写 也就是jal不会污染 
