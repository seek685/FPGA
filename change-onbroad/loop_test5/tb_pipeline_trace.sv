...
module tb_pipeline_trace;
    ...
    // Behavioral DRAM write model for stack and test-counter stores.
    always @(posedge clk) begin
        if (!rst && perip_wen && perip_addr >= 32'h8010_0000 && perip_addr < 32'h8014_0000) begin
            case (perip_mask)
                2'b10: dram[perip_addr[17:2]] <= perip_wdata;
                2'b01: begin
                    if (perip_addr[1])
                        dram[perip_addr[17:2]] <= {perip_wdata[15:0], dram[perip_addr[17:2]][15:0]};
                    else
                        dram[perip_addr[17:2]] <= {dram[perip_addr[17:2]][31:16], perip_wdata[15:0]};
                end
                default: begin
                    case (perip_addr[1:0])
                        2'b00: dram[perip_addr[17:2]] <= {dram[perip_addr[17:2]][31:8], perip_wdata[7:0]};
                        2'b01: dram[perip_addr[17:2]] <= {dram[perip_addr[17:2]][31:16], perip_wdata[7:0], dram[perip_addr[17:2]][7:0]};
                        2'b10: dram[perip_addr[17:2]] <= {dram[perip_addr[17:2]][31:24], perip_wdata[7:0], dram[perip_addr[17:2]][15:0]};
                        2'b11: dram[perip_addr[17:2]] <= {perip_wdata[7:0], dram[perip_addr[17:2]][23:0]};
                    endcase
                end
            endcase
        end
    ...
    always @(posedge clk) begin
        ...
        if (!rst && perip_wen && (perip_addr >= 32'h8020_0000)) begin
            $display("MMIO t=%0t addr=%08h data=%08h mask=%b", $time, perip_addr, perip_wdata, perip_mask);
            if (perip_addr == 32'h8020_0050 && perip_wdata == 32'hffff_ffff)
                $finish;
        end
        if (!rst && perip_wen && (perip_addr == 32'h8010_0020))
            $display("STACK t=%0t data=%08h", $time, perip_wdata);
        if (!rst && !low_seen && dut.pc[31:28] != 4'h8) begin
            low_seen = 1'b1;
            $display("LOWPC t=%0t pc=%08h ra=%08h dram8=%08h", $time, dut.pc, dut.u_regfile.regs[1], dram[8]);
        end
    end
    ...
    initial begin
        ...
        $display("FINAL pc=%08h ra=%08h dram8=%08h", dut.pc, dut.u_regfile.regs[1], dram[8]);
        $finish;
    end
endmodule
